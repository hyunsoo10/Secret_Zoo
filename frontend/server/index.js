...
async function main() {
   ...
   */
  const addRoom = (roomName, playerId, socket) => {
    rooms[roomName] = { ...roomInfo };
    rooms[roomName].roomName = roomName;
    rooms[roomName].players.push(Player(playerId,socket));
   ...
   * @param {string} id 유저 id
   */
  const addPlayer = (roomName, playerId, socket) => {
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("start", () => {
      ...
      for (let k = 0; k < 6; k++) {
        io.to(rooms[room].player[k].playerId).emit(
          "game start",
          rooms[room].player[k].hand
        );
      }
      console.log(rooms);
    });
